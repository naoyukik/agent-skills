...
本スキルの適用範囲は Step 4 までであり、実装開始は対象外とする。
...
実装提案・コード編集・パッチ作成の前に、必ず Step 1 から順に完了させること。

- Step 3 の質問が未解決のまま Step 4 に進んではならない。  
...
- **必須タイミング**:
  - Step 1 完了時: 要求の再記述、制約、成功条件、未確定事項の整理
  - Step 3 開始前: 何が未確定で、何を質問しないと設計判断できないかの棚卸し
  - Step 4 開始前: 比較案の軸、主リスク、推奨理由の妥当性確認
- **Step 2 の扱い**:
...
- 未確定事項リスト（Step 3 で回収する前提）

### Step 2: Codebase Exploration (既存実装調査)

**目的**: 既存コードと設計慣習を把握する。  
**実施内容**:

- **IDE 統合機能の優先活用 (JetBrains MCP/IDE Index MCP)**:
  ...
  - `ide_call_hierarchy`: メソッドを遡り、影響のあるコードを検索・確認する
- **専門サブエージェントへの委譲**:
...
### Step 3: Clarifying Questions (曖昧さ解消)

**目的**: 設計判断に必要な前提を確定する。  
**実施内容**:

- Step 1/2 の結果から、判断不能点を体系化する。
  - エッジケース
  - エラー処理方針
  - 互換性（既存仕様との共存/置換）
  - 性能・セキュリティ要件
...
- ユーザー回答（または明示的委任）を得るまで、Step 4 へ進まない。

### Step 4: Architecture Design (設計案比較)
